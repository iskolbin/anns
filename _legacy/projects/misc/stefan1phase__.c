...
ANNS_CND(A1d) {anns_instance_t *ai = instance; return x[0] < s_current ? (0.5 * (ai->step) * (C_K/C_C) * u_xx[0][0] - u[0] + v[0]) : u[0] - C_To;}
ANNS_CND(B1d_left) {return u[0] - C_Th;}
ANNS_CND(B1d_front) {return u[0] - C_Tm;}

ANNS_CND_G(A1d_a) {anns_instance_t *ai = instance; return x[0] < s_current ? (0.5 * (ai->step) * (C_K/C_C) * u_axx[0] - u_a) : u_a;}
...
static void ai_update_1d(anns_instance_t *ai, double *x, int i, double tau) {
    ...
    for (p = 0; p < ai->conds[0]->m; p++) {
        ...
        if (point < s_current) {
            ...
            ac->v[0][p][0] = 0.5 * (ai->step) * (C_K/C_C) * ac->u_xx[0][0] - ac->u[0];
        } else {
            ac->v[0][p][0] = 0;
        }
    }
