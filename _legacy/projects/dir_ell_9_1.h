...
int data[] = {
    PJ_DIRECT, PJ_ELLIPTIC_PROBLEM, 9, 1,

    FUNCTIOAL_SQUARE, 2, 3, 1, 0,

    50, 2,
    10, 0,
    10, 0,

    NORMALIZED | RADIAL | CAUCHY_FUNCTION, 16, 2, 0,  2, 0, 1,
};


double bounds[] = {
    GP_UNIFORM_SOLID_CUBOID,  0.0, 1.0, 0.1, 1.0, // A
    GP_UNIFORM_SOLID_CUBOID,  0.0, 0.0, 0.1, 1.0,
    GP_UNIFORM_SOLID_CUBOID,  1.0, 1.0, 0.1, 1.0,
//    GP_GRID_2D, 1, 0.0, 0.0, 20, 0.1, 0.9,
//    GP_GRID_2D, 1, 1.0, 1.0, 20, 0.1, 0.9,
};
...
ANNS_FUN(A_f1) {
    ...
    return K1 + K2;}
ANNS_FUN(A_f2) {return -x[1] / ((1-x[0])*tan(C_alpha) + C_theta);}
...
void ai_initann(anns_instance_t *ai, double *x) {
    ...
    for (t = 0; t < ai->nnets; t++) {
        for (i = ai->I[t]; i < ai->I[t+1]; ) {
            ...
            x[i++] = uniform(-0.25, 1.55);       // Центры
            x[i++] = uniform(-0.15, 1.55);       // Центры
        }
    }
}
