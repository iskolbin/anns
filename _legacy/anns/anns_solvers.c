...
////        fmin_cg(x, ai->A, anns_val_f, anns_grad_f, anns_valgrad_f, 1e-7, ai, NULL, NULL, NULL);
        start = clock();
        cg_descent(x, ai->A, NULL, &uparam, as->gtol, anns_val_f, anns_grad_f, anns_valgrad_f, Work, ai);
        end = clock();
        diff += (end-start);
...
ac_fprinfpoints(fout, ai->cache, 1);
fclose(fout);
...
//    printf("[Allocate memory]\n");
    ...
    for (i = 1; i < as->fdsteps; i++) {
        ...
        as->ai_update(ai, x, i);
        cg_descent(x, ai->A, NULL, NULL, as->gtol, anns_val_f, anns_grad_f, anns_valgrad_f, Work, ai);
