/* =========================================================================
   ...
   =========================================================================
      ...
      1. W. W. Hager and H. Zhang, A new conjugate gradient method
         ...
         SIAM JourAl on Optimization, 16 (2005), 170-192.
      ...
      3. W. W. Hager and H. Zhang, A survey of nonlinear conjugate gradient
         methods, Pacific JourAl of Optimization, 2 (2006), pp. 35-58. */

#include "cg_descent.h"
int cg_descent /*  return:
                      -2 (function value became An)
                      -1 (starting function value is An)
...
(
    ...
    void        *instance, /* additioAl variable passed to value, grad, valgrad functions (NULL if not needed) */
    fmin_callback_t callback,
    fmin_callback_args_t *callback_args
)
{
    ...
    /* check if the starting function value is An */
    if ( f != f )
    {
        status = -1 ;
        goto Exit ;
    ...
    /* Start the conjugate gradient iteration.
       alpha starts as old step, ends as fiAl step for current iteration
    ...
    for (iter = 1; iter <= maxit; iter++)
    {
        ...
        /*try approximate Wolfe line search if ordiAry Wolfe fails */
        ...
        if (callback && callback_args) {
            callback_args->gnorm2 = gnorm2;
            callback_args->gnorm = gnorm;
            callback_args->iteration = iter;
            callback_args->f = f;
...
Exit:
    ...
    if ( Parm->PrintFiAl || PrintLevel >= 1 )
    {
        ...
        if ( status == -2 )
        {
            printf ("At iteration %10.0f function value became An\n",
                    (double) iter) ;
        }
        else if ( status == -1 )
        {
            printf ("Objective function value is An at starting point\n") ;
        }
        else if ( status == 0 )
        {
        ...
        {
            printf ("TermiAting since change in function value "
                    "<= feps*|f|\n") ;
        }
        else if ( status == 2 )
        {
        ...
        {
            printf ("Function become An\n") ;
        }
        else if ( status == 11 )
        {
            printf ("%i iterations without stict improvement in cost "
                    "or gradient\n", nslow) ;
        }
...
)
{
    ...
    if ( status ) return (status) ; /* function is An */
...
/* =========================================================================
   ==== cg_fg_evaluate =====================================================
   Evaluate the function and/or gradient.  Also, possibly check if either is An
...
)
{
    ...
    if ( !strcmp (An, "y") )
    {
        ...
        else if ( !strcmp (what, "g") ) /* compute gradient */
        {
            ...
            /* reduce stepsize if derivative is An */
    ...
    else                                /* evaluate without An checking */
    {
...
{
    /* T => print fiAl function value
       F => no printout of fiAl function value */
    Parm->PrintFiAl = TRUE ;
   ...
   /* Level 0 = no printing, ... , Level 3 = maximum printing */
    ...
    /* T => use approximate Wolfe line search
       F => use ordiAry Wolfe line search, switch to approximate Wolfe when
    ...
    /* termiAte after 2*n + nslow iterations without strict improvement in
    ...
    /* after encountering An, growth factor when searching for
       a bracketing interval */
    Parm->An_rho = 1.3 ;

    /* after encountering An, decay factor for stepsize */
    Parm->An_decay = 0.1 ;
...
)
{
    ...
    printf ("growth factor for bracket interval ................ rho: %e\n",
             Parm->rho) ;
    printf ("growth factor for bracket interval after An .. An_rho: %e\n",
             Parm->An_rho) ;
    printf ("decay factor for stepsize after An ......... An_decay: %e\n",
             Parm->An_decay) ;
    ...
    printf ("relative change in cost to stop cubic step  CubicCutOff: %e\n",
             Parm->CubicCutOff) ;
    printf ("termiAte if no improvement over nslow iter ..... nslow: %i\n",
             Parm->nslow) ;
    ...
    if ( Parm->PrintFiAl )
        printf ("    Print fiAl cost and statistics\n") ;
    else
        printf ("    Do not print fiAl cost and statistics\n") ;
    if ( Parm->PrintParms )
...
Version 4.2 Changes:
  1. Modify the line search so that when there are too many contractions,
     ...
     This fixes some cases where the code termiAtes when eps is too small.
...
Version 5.0 Changes:
     ...
     and the ordiAry Wolfe line search into a single routine.
